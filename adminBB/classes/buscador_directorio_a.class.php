...
<?php
	...
	$txt_criterio = $criterio;
	$sql.=$pez;
	$sql.=" ORDER BY homeopop_directorio.nombre ASC LIMIT $pagina,$this->regxpag";
	$rs=mysql_query($sql);
...
?>
<td style=" border-bottom: 1px dotted; width: 10px; padding-right:5px">
	<a href="<?php echo $self ?>.php?ruta=if_directorio_a.php&rubro=<?php echo $row2[0] ?>">Editar</a>
</td>

	<td style=" border-bottom: 1px dotted; width: 0px">
		<a href="<?php echo $self ?>.php?ruta=borra.php&borra=<?php echo $borra ?>&rubro=<?php echo $row2[0] ?>" onclick="return confirm('Los datos de <?php echo $row2[1]?> se perderán, ¿seguro?');">Borrar</a><br>		
	</td>
