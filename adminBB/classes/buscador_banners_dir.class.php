...
<?php
	...
	$txt_criterio = $criterio;
	$sql.=$pez;
	$sql.="ORDER BY nombre LIMIT $pagina,$this->regxpag";
	$rs=mysql_query($sql);
	...
	?>
				...
				<div style="margin:0px auto; text-align:center"><table border="0"><tr>
		<?php
					for($i=0; $i<=$total;$i++){
		?>
...
?>
	<td style=" border-bottom: 1px dotted; width: 0px">
		<a href="<?php echo $self ?>.php?ruta=borra.php&borra=<?php echo $borra ?>&rubro=<?php echo $row2[2] ?>">Borrar</a><br>
	</td>
