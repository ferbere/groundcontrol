...
<?php
	...
	function pagina($pag,$sql,$pez,$set,$fili,$celdas,$self) {		
		if(isset($_GET['criterio'])){
			$criterio=$_GET['criterio'];
		}
...
<?php
		...
		?>
						<td style="text-align: left; border-bottom: 1px dotted; width: <?php echo $percent ?>%; height: 30px;">
							<a href="gadgets/genealogia/<?php echo $set ?>?rubro=<?php echo $row2[0] ?>&fili=<?php echo $fili ?>&parentesco=<?php echo $row2['genero'] ?>"><?php echo $row2[$i] ?></a>
						</td>
<?php
					}
				}
...
<?php		
		...
		function pie($pag,$sql,$pez,$self,$fili) {
			if(isset($_GET['criterio'])){
				$criterio=$_GET['criterio'];
			}else{
				$criterio='';
...
<?php
	...
	echo "<a href='".$self.".php?ruta=".$ruta."&fili=".$fili."&criterio=".$criterio."&pag=$i'>".($i+1)."</a>";
						if ($i!=($pagxlet-1)){
							echo "&nbsp;|&nbsp;";
						}
					}
